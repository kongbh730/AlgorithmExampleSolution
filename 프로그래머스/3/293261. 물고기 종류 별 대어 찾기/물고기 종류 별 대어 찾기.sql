...
SELECT FI1.ID, FNI1.FISH_NAME, FI1.LENGTH
FROM FISH_INFO FI1 JOIN FISH_NAME_INFO FNI1 ON FI1.FISH_TYPE = FNI1.FISH_TYPE
JOIN (
  SELECT FI2.FISH_TYPE, MAX(FI2.LENGTH) AS MAX_LENGTH
  FROM FISH_INFO FI2
  GROUP BY FI2.FISH_TYPE
) TEMP_TABLE ON FI1.FISH_TYPE = TEMP_TABLE.FISH_TYPE AND FI1.LENGTH = TEMP_TABLE.MAX_LENGTH
ORDER BY FI1.ID;
